...
def solve_with_co2_constraint(co2_limit=None, include_storage=True, min_renewable_share=None):
    ...
    if co2_limit is not None:
        # Extract the shadow price of the CO2 constraint (€/ton CO2)
...
# Define the original scenarios as tuples of (CO2 constraint, RE target)
original_scenarios = [
...
# Run scenarios
results_list = []
scenarios = []  # This will hold scenario labels

for co2_limit, re_target in original_scenarios:
    ...
    scenarios.append(scenario_label)  # Add the label to the scenarios list
...
# Using the original_scenarios list instead of the scenarios list
high_re_scenario = original_scenarios[high_re_scenario_index]
...
shadow_prices = [r.get('co2_shadow_price', 0) for r in results_list]

# Create DataFrame with shadow prices
shadow_price_df = pd.DataFrame({
    'Scenario': [r['scenario_label'] for r in results_list],
    'CO2 Constraint (Mt)': [r['co2_constraint']/1000000 if r['co2_constraint'] is not None else float('inf') for r in results_list],
    'Shadow Price (€/ton CO2)': shadow_prices,
    'Abatement Cost (€/ton CO2)': [-p for p in shadow_prices],  # Convert to positive for intuitive interpretation
    'Renewable Target (%)': [r['re_target']*100 if r['re_target'] is not None else 0 for r in results_list]
})

# Plot shadow CO2 prices (as abatement costs - positive values)
plt.figure(figsize=(12, 8))
sns.barplot(x='Scenario', y='Abatement Cost (€/ton CO2)', data=shadow_price_df)
plt.xticks(rotation=45, ha='right')
plt.title('CO2 Abatement Costs by Scenario', fontsize=16)
plt.ylabel('Abatement Cost (€/ton CO2)', fontsize=14)
plt.grid(axis='y', linestyle='--', alpha=0.7)
plt.tight_layout()
plt.savefig('co2_abatement_costs.png', dpi=300)
plt.show()

# Alternative visualization by CO2 constraint
plt.figure(figsize=(10, 6))
for co2_constraint in sorted(set(shadow_price_df['CO2 Constraint (Mt)'])):
    if not np.isinf(co2_constraint):  # Skip scenarios with no CO2 constraint
        subset = shadow_price_df[shadow_price_df['CO2 Constraint (Mt)'] == co2_constraint]
        plt.plot(subset['Renewable Target (%)'], subset['Abatement Cost (€/ton CO2)'], 
                'o-', linewidth=2, label=f'CO2 limit: {co2_constraint} Mt')

plt.xlabel('Renewable Target (%)', fontsize=14)
plt.ylabel('Abatement Cost (€/ton CO2)', fontsize=14)
plt.title('CO2 Abatement Costs vs. Renewable Targets', fontsize=16)
plt.grid(True, linestyle='--', alpha=0.7)
plt.legend()
plt.tight_layout()
plt.savefig('co2_abatement_by_target.png', dpi=300)
plt.show()

# Heatmap to visualize the relationship
pivot_df = shadow_price_df.pivot_table(
    index='CO2 Constraint (Mt)', 
    columns='Renewable Target (%)', 
    values='Abatement Cost (€/ton CO2)'
)

plt.figure(figsize=(10, 8))
sns.heatmap(pivot_df, annot=True, cmap='viridis', fmt='.1f', 
            linewidths=.5, cbar_kws={'label': 'Abatement Cost (€/ton CO2)'})
plt.title('CO2 Abatement Costs by Constraint and Renewable Target', fontsize=14)
plt.tight_layout()
plt.savefig('co2_abatement_heatmap.png', dpi=300)
plt.show()
