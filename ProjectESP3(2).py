...
df_capacity = pd.DataFrame({
    ...
    'Biomass': [r['generators']['biomass']['p_nom_opt'] for r in results],
    'Hydro': [r['generators']['hydro']['p_nom_opt'] for r in results]
})
...
df_generation = pd.DataFrame({
    ...
    'Biomass': [r['generators']['biomass']['p_sum'] for r in results],
    'Hydro': [r['generators']['hydro']['p_sum'] for r in results]
})
...
df_capacity_factors = pd.DataFrame({
    ...
    'Biomass': [r['generators']['biomass']['capacity_factor'] for r in results],
    'Hydro': [r['generators']['hydro']['capacity_factor'] for r in results]
})
...
def plot_capacity_variability():
    ...
    mean_capacities = df_capacity.mean()
    std_capacities = df_capacity.std()
    
    # Create bar plot with error bars
    tech_names = ['Onshore Wind', 'Solar', 'OCGT', 'Biomass', 'Hydro']
    ...
    ax.set_title('Average Optimal Capacity and Variability across Weather Years (2000-2015)', fontsize=16)
    ax.set_xticks(x)
...
def plot_capacity_factor_variability():
    ...
    plt.ylabel('Capacity Factor', fontsize=14)
    plt.title('Distribution of Capacity Factors across Weather Years (2000-2015)', fontsize=16)
...
def plot_generation_mix_variability():
    ...
    for tech in ['Onshore Wind', 'Solar', 'OCGT', 'Biomass', 'Hydro']:
    ...
    colors = {'Onshore Wind': 'blue', 'Solar': 'orange', 'Biomass': 'green', 
              'OCGT': 'brown', 'Hydro': 'pink'}
    
    for tech in ['Onshore Wind', 'Solar', 'Biomass', 'OCGT', 'Hydro']:
    ...
    plt.title('Average Electricity Generation Mix (2000-2015)', fontsize=16)
    plt.tight_layout()
...
summary_capacity = pd.DataFrame({
    'Technology': ['Onshore Wind', 'Solar', 'OCGT', 'Biomass', 'Hydro'],
    'Mean Capacity (MW)': [df_capacity[tech].mean() for tech in ['Onshore Wind', 'Solar', 'OCGT', 'Biomass', 'Hydro']],
    'Std Dev (MW)': [df_capacity[tech].std() for tech in ['Onshore Wind', 'Solar', 'OCGT', 'Biomass', 'Hydro']],
    'Min Capacity (MW)': [df_capacity[tech].min() for tech in ['Onshore Wind', 'Solar', 'OCGT', 'Biomass', 'Hydro']],
    'Max Capacity (MW)': [df_capacity[tech].max() for tech in ['Onshore Wind', 'Solar', 'OCGT', 'Biomass', 'Hydro']],
